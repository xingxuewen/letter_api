        ...
        <div class="result-yes" style="display:block">
            ...
            <section>
                ...
                <div id="products-list">
                    @if(isset($result['list'])) @foreach($result['list'] as $key=>$item)
                        <dl>
    ...
    <div class="result-bottom">
        @if(isset($result['list']))
            @if(isset($product['list']) && !empty($product['list']))
                <h3>根据您的综合评定，为您推荐以下贷款组合，申请<span style="font-size: .28rem;color: #FE5C0D;">3</span>个以上成功率提高到<span style="font-size: .28rem;color: #FE5C0D;">98%</span>。</h3>
            @endif
        @else
        <!--无匹配结果-->
            @if(isset($product['list']) && !empty($product['list']))
                <h3>和您资质相似的人都申请了以下贷款产品，申请<span style="font-size: .28rem;color: #FE5C0D;">3</span>个以上成功率提到高<span style="font-size: .28rem;color: #FE5C0D;">98%</span>。</h3>
            @endif
        @endif
                ...
                @foreach($product['list'] as $key=>$item)
                    <div class="product_list" data-productid="{{ $item['platform_product_id'] }}">
                        <dl>
                            ...
                            <dd>
                                <h3>{{ $item['platform_product_name'] }} @if($item['tag_name'])
                        ...
                        <div class="product_list_btm">
                            ...
                            <div class="product_list_btm_center">
                                ...
                                @if($item['interest_alg'] == 1)
                                    <p>月利率 ：{{ $item['interest_rate'] }}</p>
                                @elseif($item['interest_alg'] == 0)
                                    <p>日利率 ：{{ $item['interest_rate'] }}</p>
                                @else
                                    <p>利率 ：{{ $item['interest_rate'] }}</p>
                                @endif
                            </div>
                            <div class="product_list_btm_right"><span class="apply"
                                                                      data-platformid="{{ $item['platform_id'] }}"
                                                                      data-productid="{{ $item['platform_product_id'] }}"
                                                                      data-title="{{ $item['platform_product_name'] or  ''}}"
                                                                      data-typenid="{{ $item['type_nid'] or ''}}"
                                                                      data-mobile="{{ $item['mobile'] or ''}}">申请</span>
                            </div>
                        </div>
                        {{--vip标识--}} @if(isset($item['is_vip_product']) && $item['is_vip_product'] == 1)
                            <i class="vip_icon"></i> @endif {{--速贷优选标识--}} @if(isset($item['is_preference']) && $item['is_preference'] == 1)
                            <i class="choose_icon"></i> @endif
                    </div>
                @endforeach
            </div>
