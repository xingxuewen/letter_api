...
use Monolog\Handler\SlackHandlerTest;
...
class CreateVipOrderAction extends AbstractHandler
{
    ...
    {
        if ($this->createVipOrder_new($this->params)) {
            $this->setSuccessor(new BankCardAction($this->params, $this->backInfo));
            return $this->getSuccessor()->handleRequest();
        } else {
            return $this->error;
     ...
     *  by xuyj v3.2.3
     */
    private function createVipOrder_new($params = [])
    {
        logInfo("createVipOrder_new", $params);
        $payType = $params['pay_type'];
        //logInfo("createVipOrder_new func line 104  = ".$params['subVip']['vip_type']);
        ...
        switch ($payType) {
            ...
            case 4: // 快捷支付 汇聚
                ...
                if(strcmp($params['isBangCard'],"1")==0){
                    ...
                    logInfo("zzzzzzzzzzzzzzzzzzzzzzzzzzz");
                    $bankCardInfo = UserBankCardFactory::getBankCardInfo($params['bankcard_id'], $params['user_id']);
                    logInfo("createVipOrder_new_bankCardInfo", $bankCardInfo);
                    if(intval($bankCardInfo['huiju_paycount'])==0){
                        $res = $this->quickPayment_huiju($params,$bankCardInfo);
                    }else if(intval($bankCardInfo['huiju_paycount'])>0){
                        $res = $this->quickPayment_huiju_sec($params);
    ...
    private function wechatpay_mini($params=[]){
        ...
        $bankCardInfo = [];
        logInfo("hhhhhhhhhhhhhhhhhhhhhh----", $params);
      ...
      //  $back['fcallbackurl'] = AppService::HUIJU_CALLBACK_URL . AppService::API_URL_HUIJU_SYN;
        ...
        $reOrder = PaymentStrategy::getUserOrderParams_new($params, $orderOtherParams);
        $reOrder['card_num']="0";
        $reOrder['subtype']  = urldecode($subvipinfo);//$params['subVip'];

        logInfo("LLLLLLLLLLLLLLLLLLLLLLLL---", ['subVip_vip_type', $params['subVip']['vip_type']]);
    ...
    private function quickPayment($params = [])
    {
        ...
        $back['orderNum'] = $params['order_id'];
        //调用易宝订单
        logInfo("0000000000000000----", $params);
        $amount = UserVipFactory::getReVipAmountByNid($params['subtypeNid']);
        $otherParams = UserVipStrategy::getYibaoOtherParamsByParam($params);
        ...
        $orderOtherParams = UserVipStrategy::getUserOrderOtherParamsByParam($params);
        $reOrder = PaymentStrategy::getUserOrderParams($params, $orderOtherParams);
        logInfo("xxxxxxxxxxxxxxxxxxxxxx");
    ...
    private function quickPayment_huiju($params = [],$bankCardInfonew)
    {
        //如果是银行卡支付
        $bankCardInfo = [];
        logInfo('quickPayment_huiju', $params);
        $subvipinfo = \GuzzleHttp\json_encode($params['subVip']);
        $pcvv2="";
        $pavatime = "";
        if(strcmp($params['cardtype'],"2")==0){
      ...
      // 首次使用汇聚支付的卡
        ...
        if(strcmp($params['cardtype'],"2")==0 && strcmp($params['isBangCard'],"1")==0){
            if(strcmp($params['cvv2'],"0")==0){
                ...
                $huiJuparams['s6_CVV2'] =  $pcvv2;//$params['cvv2'];
            }
            ...
            logInfo("SSSSSSSSSSSSSSSSSSSSSSSSSSS222222");
            ...
            if(strcmp($params['avatime'],"0")==0){
                $huiJuparams['s5_BankCardExpire'] = $pavatime;
            }else if(strlen($params['avatime'])>=5){
                $huiJuparams['s5_BankCardExpire'] = $params['avatime'];//$avatime;
            }else{
                $huiJuparams['s5_BankCardExpire'] = $params['avatime'];//$avatime;
            }
        }
        logInfo("ppppppppppppppppppp----", $huiJuparams);
        // 参数排序
        ksort($huiJuparams);
        //生成签名
      //  $hmcVal = urlencode($this->hmacRequest($huiJuparams,"9aec0efceb804391842838bdc420ebbd"));
        ...
        if (empty($ret)) {
            $this->error = ['error' => RestUtils::getErrorMessage(1136), 'code' => 99991];
            return false;
        }
        if(strcmp($ret['ra_Status'],"100")!=0 && strcmp($ret['ra_Status'],"102")!=0){
            $errorMsg = PaymentFactory::getErrorMsg($ret['ra_Status'], '交易失败，请检查信息或联系客服~');
            $this->error = ['error' => $errorMsg, 'code' => 99991];
            return false;
        }

        logInfo("=====HUI JU return value : ", $ret);
        $huiJuparams['productdesc'] = "【会员充值】";
        ...
        $back['subvipinfo'] = urlencode($subvipinfo);
        //创建订单
        //汇聚支付唯一标识
        $params['payment_nid'] = UserVipConstant::PAYMENT_TYPE_HUIJU;
        $params['amount'] = $huiJuparams['q2_Amount'];
        $orderOtherParams = UserVipStrategy::getUserOrderOtherParamsByParam_new($params);
        $reOrder = PaymentStrategy::getUserOrderParams_new($params, $orderOtherParams,$subvipinfo);
        logInfo("getUserOrderOtherParamsByParam  : ", $reOrder);
        $createOrder = PaymentFactory::createOrder($reOrder);
        if (!$createOrder) {
            $this->error = ['error' => "交易失败，请检查信息或联系客服", 'code' => 99991];
            return false;
        }
        //将返回结果赋值
        $this->backInfo = $back;
...
//        return json_decode(file_get_contents($url, false, $context), true);
    ...
    private function quickPayment_huiju_sec($params = [])
    {
        //如果是银行卡支付
        $bankCardInfo = [];
        logInfo('quickPayment_huiju_sec', $params['subVip']);
        $subvipinfo = \GuzzleHttp\json_encode($params['subVip']);
        // 4=>汇聚
        if($params['pay_type']==4){
            $bankCardInfo = UserBankCardFactory::getBankCardInfo($params['bankcard_id'], $params['user_id']);
        ...
        $huiJuparams = PaymentStrategy::orderHuiJuParams_new_Sec($params, $otherParams,$bankCardInfo['huijusignid']);//UserVipStrategy::orderYibaoParams($order);
        if(strcmp($params['card_type'],"2")==0){

            $huiJuparams['s6_CVV2'] = $params['cvv2'];
            if(strcmp($params['avatime'],"0")==0){
                $huiJuparams['s5_BankCardExpire'] ="0";
            }else{
                $huiJuparams['s5_BankCardExpire'] =$params['avatime'];//$avatime;
            }
        }
    ...
    private function quickPayment_huiju_sec_report($params = [])
    {
        //如果是银行卡支付
        $bankCardInfo = [];
        logInfo('quickPayment_huiju_sec_report', $params['subVip']);
        $subvipinfo = \GuzzleHttp\json_encode($params['subVip']);
        // 4=>汇聚
        if($params['pay_type']==4){
            $bankCardInfo = UserBankCardFactory::getBankCardInfo($params['bankcard_id'], $params['user_id']);
