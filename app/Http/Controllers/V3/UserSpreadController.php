...
class UserSpreadController extends Controller
{
    ...
    public function fetchOneloanInfo(Request $request)
    {
        ...
        //极速贷类型标识
        $data['quickloanNid'] = $request->input('quickloanNid', 'sudaizhijia');
        ...
        $add_product_desc = SpreadStrategy::getOneloanInfo($params);
        ...
        //极速贷
        $quickTypeId = QuickloanFactory::fetchQuickloanConfigTypeIdByNid($data);
        $quickConfig = QuickloanFactory::fetchQuickloanConfigById($quickTypeId);
        if ($quickConfig) {
            $quickConfig['button_subtitle'] = $add_product_desc ? $add_product_desc : '';
        }

        $datas['spread'] = $spread ? $spread : RestUtils::getStdObj();
        $datas['quickloan'] = $quickConfig ? $quickConfig : RestUtils::getStdObj();
