...
class UserSpreadController extends Controller
{
    ...
    public function fetchOneloanInfo(Request $request)
    {
        ...
        //用户贷款额度
        $loanMoney = UserSpreadFactory::fetchBasicInfo($data);
        //贷款额度
        $datas['money'] = $loanMoney ? $loanMoney['money'] . '' : '5000';
        ...
        //分析差值
        $diffCount = $diffIds ? count($diffIds) : 0;
        ...
        //显示差值
        if (empty($diffCount)) {
            $datas['add_product_num'] = $counts . '';
            $datas['add_product_num_sign'] = 1;
        } else {
            $datas['add_product_num'] = $diffCount . '';
            $datas['add_product_num_sign'] = 0;
        }
