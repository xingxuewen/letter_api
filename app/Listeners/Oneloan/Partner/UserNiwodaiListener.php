 ...
 */
class UserNiwodaiListener extends AppListener
{
    ...
    {
        try {
            $type = UserSpreadFactory::fetchSpreadTypeByNid(SpreadNidConstant::SPREAD_NIWODAI_NID);
            if (!empty($type)) {
                ...
                if (!$spreadLogInfo || 2 == $spreadLogInfo['status']) {
                    ...
                    //logInfo('你我贷', ['data' => $spread]);
                    $this->pushNiwodaiData($spread);
                }
            }
        } catch (\Exception $exception) {
            logError('你我贷发送失败-catch');
    ...
    {
        $typeNid = SpreadNidConstant::SPREAD_NIWODAI_NID;
        //24小时限制
...
//        return false;
    ...
    {
        if ($data['accumulation_fund'] != '000' or $data['has_creditcard'] == 1) {
            return true;
        }
