 ...
 * 融时代延迟推送
 *
 * Class UserSpreadRongshidaiBatchListener
 * @package App\Listeners\V1
 */
class UserSpreadRongshidaiBatchListener extends AppListener
{
    ...
    {
        if ($spread['type_nid'] == SpreadNidConstant::SPREAD_RONGSHIDAI_NID) {
            // 推送service
            RongshidaiService::spread($spread,
                function($res) use ($spread) {
                    //是否是延迟推送【0立即推送，1延迟推送】
                    $spread['batch_status'] = 1;
                    $spread = RongshidaiCallback::handleRes($res, $spread);
