 ...
 * 大智金服延迟推送
 *
 * Class UserSpreadDazhiBatchListener
 * @package App\Listeners\V1
 */
class UserSpreadDazhiBatchListener extends AppListener
{
    ...
    public function handle(UserSpreadBatchEvent $event)
    {
        try{
            ...
            logError('大智金服延迟推送失败-catch', $exception->getMessage());
        }
    }
    ...
    {
        $spread_nid = SpreadStrategy::getSpreadNid(SpreadNidConstant::SPREAD_DAZHI_NID);

        if ($spread['spread_nid'] == $spread_nid) {

            $spread['age'] = Utils::getAge($spread['birthday']);
//            logError(json_encode($spread));
            // 推广service
