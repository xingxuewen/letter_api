 ...
 * @package App\Listeners\Oneloan
 */
class UserDongfangListener extends AppListener
{
    ...
    {
        try {
            $type = UserSpreadFactory::fetchSpreadTypeByNid(SpreadNidConstant::SPREAD_DONGFANG_NID);
            ...
            if (!empty($type)) {
                ...
                if (!$spreadLogInfo || 2 == $spreadLogInfo['status']) {
                    ...
                    //logInfo('东方', ['data' => $spread]);
                    $this->pushDongfangData($spread);
                }
            }
        } catch (\Exception $exception) {
            logError('东方金融发送失败-catch', $exception->getMessage());
        }
    }
     ...
     * @return bool
     */
    public function pushDongfangData($spread)
    {
        $typeNid = SpreadNidConstant::SPREAD_DONGFANG_NID;
        //24小时限制
