...
use App\Listeners\Oneloan\Partner\Callback\JibaodaiCallback;
use App\Models\Factory\CacheFactory;
use App\Models\Factory\UserSpreadFactory;
use  App\Services\Core\Platform\Jibaodai\Jibaodai\JibaodaiService;
 ...
 * 吉宝贷
 *
 * Class UserJibaodaiListener
 * @package App\Listeners\V1\Partner
 */
class UserJibaodaiListener extends AppListener
{
    ...
    {
        try {
            $type = UserSpreadFactory::fetchSpreadTypeByNid(SpreadNidConstant::SPREAD_JIBAODAI_NID);
            if (!empty($type)) {
                //查询数据
    ...
    {
        $typeNid = SpreadNidConstant::SPREAD_JIBAODAI_NID;
        //24小时限制
...
//        }
    ...
    private function nowPush($spread)
    {
        ...
        //  推送助贷网
        JibaodaiService::spread($spread,
            function ($res) use ($spread) {
                //处理结果
                //是否是延迟推送流水  0不是，1是
                $spread['batch_status'] = 0;

                JibaodaiCallback::handleRes($res, $spread);
    ...
    {
        //  有房贷/寿险且
        if (in_array($data['house_info'], ['001']) or in_array($data['has_insurance'], ['1', '2'])
        ) {
            if (in_array($data['has_creditcard'], ['1'])) {
