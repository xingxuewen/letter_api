...
class DazhiCallback
{
    ...
    public static function handleRes($res, $spread)
    {
        ...
        if (isset($res['data']['resultCode'])) {
            if ($res['data']['resultCode'] == 200) {
                $spread['status'] = 1;
                $spread['group_status'] = 1;
                $spread['response_code'] = 1;
                $spread['message']=isset($res['data']['resultMsg'])?$res['data']['resultMsg']:'成功';
            } else {
                $spread['status'] = 0;
                $spread['group_status'] = 0;
                $spread['response_code'] = 2;
                $spread['message']=isset($res['data']['resultMsg'])?$res['data']['resultMsg']:'失败';
            }
        }
