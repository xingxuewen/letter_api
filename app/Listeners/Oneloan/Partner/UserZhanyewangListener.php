...
use App\Listeners\Oneloan\Partner\Callback\ZhanyewangCallback;
 ...
 */
class UserZhanyewangListener extends AppListener
{
    ...
    {
        try {
            $type = UserSpreadFactory::fetchSpreadTypeByNid(SpreadNidConstant::SPREAD_ZHANYEWANG_NID);
            if (!empty($type))
            {
                //查询数据
            ...
            logError('展业王发送失败-catch', $exception->getMessage());
        }
    }
    ...
    {
        $typeNid = SpreadNidConstant::SPREAD_ZHANYEWANG_NID;
        //24小时限制
...
//        }
    ...
    {
        $spread['order_num']=UserSpreadFactory::OrderNum();
        // 推广service
        ZhanyewangService::spread($spread,
            function ($res) use ($spread) {
                //处理结果
                //是否是延迟推送流水  0不是，1是
                $spread['batch_status'] = 0;
                ZhanyewangCallback::handleRes($res, $spread);
            }, function ($e) {
    ...
    {
        //有房或有车或有保单或有微粒贷 或者上班族+社保+打卡工资　　　　　　
        if (in_array($data['house_info'], ['001', '002']) or in_array($data['car_info'], ['001', '002']) or in_array($data['has_insurance'],[1,2]) or $data['is_micro']==1) {
            return true;
        }
        if($data['occupation']=='001' and $data['social_security']==1 and $data['salary_extend']=='001'){
            return true;
        }
