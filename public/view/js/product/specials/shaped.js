...
//		this.handleScroll();
	...
	bindEvent:function(){
		...
		$('.product_list').off('click').on('click', function (e) {
            ...
            try {
                _czc.push(['_trackEvent', 'v1异形banner'+specialtitle, title , '进入产品详情', '1', '']);
            } catch (e) {
                console.log('cnzz错误');
            }
        ...
        $('.apply').on('click', function (e) {
            ...
            var title = $(this).data('title');
            try {
                _czc.push(['_trackEvent', 'v1异形banner'+specialtitle, title , '申请产品按钮', '1', '']);
            } catch (e) {
                console.log('cnzz错误');
            }
            _self.goApply({
	...
	getInfo:function(){
		...
		global.addLoading();
		$.ajax({
			url: api_sudaizhijia_host + "/v1/product/banner/special",
			type: "get",
            dataType: "json",
            data: {
            	"typeNid" : 'special_banner',
