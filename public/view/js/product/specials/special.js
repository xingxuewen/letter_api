var specialController = {
	init:function(){
		this.bindEvent();
		var timer = setTimeout(function(){
			$('.special_bottom').css('padding-top','2.6rem');
		},100)
	},
	bindEvent:function(){
		var _self = this;
		$('.list_item').off('click').on('click', function (e) {
            ...
            try {
                _czc.push(['_trackEvent', '分类专题'+specialTitle, title,'进入产品详情', '1', '']);
            } catch (e) {
                console.log('cnzz错误');
            }
		...
		$('.apply').on('click', function (e) {
			...
			var id = $(this).data('id');
            try {
                _czc.push(['_trackEvent', '分类专题'+specialTitle, title, '申请产品按钮','1', '']);
            } catch (e) {
                console.log('cnzz错误');
            }
            _self.goApply({
